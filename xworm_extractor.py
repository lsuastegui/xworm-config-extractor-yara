...
XWorm PE32 .NET assemblies without executing the sample.
...
Limitations
-----------
- Supports XWorm samples that implement the configuration format and
  AES-256-ECB / MD5 key-derivation scheme targeted by this extractor.
  Modified builds, future variants, or samples using a different scheme
  may fail.
...
# ── known XWorm config field order (positional in the Settings class) ──
...
def derive_key(mutex: str) -> bytes:
    """
    XWorm AES key derivation used by supported samples.
...
def build_config(decrypted: dict, b64_candidates: list[str],
                 mutex: str) -> dict:
    """
    Map decrypted values to known XWorm field names by position.
...
def extract_config(filepath: str, verbose: bool = False) -> dict:
    ...
    if best_score == 0:
        result["error"] = (
            ...
            "  • Sample uses a different or modified encryption scheme\n"
            "  • Not an XWorm sample"
        )
        return result
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="XWorm static config extractor",
